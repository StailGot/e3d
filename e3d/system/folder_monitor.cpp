...
namespace sys
{
  ...
  void folder_monitor::notify( double elapsed )
  {
    ...
    uint8_t buffer [2048] = {};
    DWORD n_bytes = 0;
    ::ReadDirectoryChangesW( _events_handles [change_event], buffer, static_cast<DWORD>(std::size(buffer)), FALSE, FILE_NOTIFY_CHANGE_LAST_WRITE, &n_bytes, nullptr, nullptr );
